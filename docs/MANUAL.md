...
11b. [Vue 2 / Nuxt 2 분석 (Phase 1)](#11b-vue-2--nuxt-2-분석-phase-1)
12. [한계](#12-한계)
13. [확장하기](#13-확장하기)
14. [테스트](#14-테스트)
15. [소스 구조](#15-소스-구조)
...
```bash
node dist/cli.js analyze --repo <dir> [--project P] [--out f.json] [--env kv.txt] [--mode development|production]
```

React/Vue 프로젝트를 자동 감지해 함께 분석합니다(동일 `IrFile[]` → 단일 그래프).
...
| `--env kv.txt` | 추가 env 값 파일(`KEY=VALUE` 줄 단위). `.env`/config 보다 우선 적용 |
| `--mode <m>` | Vue/Nuxt `config/<m>.json` env 선택(기본 `development`). React엔 무영향 |
...
## 11b. Vue 2 / Nuxt 2 분석 (Phase 1)

`package.json`에 `vue`/`nuxt`가 있으면 Vue 리졸버(`VueResolver`)가 적용됩니다(React 리졸버와 함께
한 번의 `analyze`에서 프로젝트별 자동 디스패치). Phase 1은 **영향도 체인**을 추출합니다:

```
SCREEN(page) → dispatch → Vuex action → http → API 노드 → 백엔드 조인
```

**`.vue` 해석 방식**: 커스텀 `ts.CompilerHost`가 각 `foo.vue`를 가상 `foo.vue.ts`(=`<script>` 블록,
원본과 라인 1:1 정렬)로 투영하고, `.vue` import를 `.vue.ts`로 모듈 해석합니다. 덕분에 `.vue`↔`.js`(store/apis)
교차 파일 심볼 해석이 되어 React와 **동일한 TypeChecker 기반 정밀 해석**(URL folding, 래퍼 추적)을 씁니다.

**라우트(화면)**: Nuxt 파일기반 `pages/**.vue`. `_id.vue`→`{}`, `index.vue`→상위 경로, 중첩 폴더.
화면은 페이지의 default export 컴포넌트.

**외부 API**: 주입형 `this.$axios`/`window.$nuxt.$axios`/`app.$axios`(체인 끝이 `$axios`)와 호출형
`this.$axios(path)`(GET), 단축 `$get/$post`, import된 `axios`를 인식. baseURL은 `nuxt.config.js`의
`['@nuxtjs/axios',{baseURL}]`에서 읽어 주입하고, `process.env.API_VERSION` 등은 `config/<mode>.json`에서
folding(`--mode`로 선택, 기본 development). 조인은 경로만 쓰므로 baseURL 미해석이어도 endpoint는 해석됨.
URL의 `?${...}` 쿼리는 정규화 단계에서 제거됩니다.

**Vuex(모듈 모드)**: `store/<path>.js` → 네임스페이스(`store/login.js`→`login`,
`store/products/list.js`→`products/list`, `store/index.js`→root).
- **모듈 노드** `store:vuex:<ns>`(STORE, `resourceType:'vuex'`) — getter/state read 대상.
- **액션 노드** `store:vuex:<ns>#<action>`(STORE, `resourceType:'vuex-action'`) — 각 `actions` 키. 액션
  본문을 walk해 `http` 엣지(액션→API, apis/* 래퍼 추적 포함)를 만든다. 이 노드가 `페이지→액션→API`
  체인의 핵심.
- 사용 매핑: `this.$store.dispatch('ns/action')`·`store.dispatch(...)`·구조분해 `dispatch(...)` 및
  `mapActions({local:'ns/action'})`/`mapActions('ns',[...])`/`mapActions([...])` → 액션 노드로 `dispatch` 엣지.
  `mapGetters/mapState`·`$store.getters[...]` → 모듈 노드로 `store:read`. (mutations/commit은 Phase 1 제외.)

**Phase 1 미포함(→ Phase 2)**: 컴포넌트 렌더 그래프(Pug 템플릿의 자식 컴포넌트 사용)와 Pug 화면
와이어프레임(`screens`). 따라서 Vue 컴포넌트 노드는 생성되지만 `render` 엣지와 Pug `LayoutNode`는 아직 없음.

**한계(Vue 고유)**: `$axios`는 주입형이라 import 없이 구조적으로만 탐지; 동적 컴포넌트
`component(:is=...)`/완전 동적 URL은 미해석; `config` 패키지의 비표준 env 위치는 추가 설정 필요.
...
ts-analyzer/src/
  ...
  resolver/
    ...
    irBuilder.ts        # React 패스 오케스트레이션 → IrFile[]
    vue/                 # Vue 2 / Nuxt 2 리졸버 (Phase 1)
      sfc.ts               # .vue → <script>/<template> 분리 (라인 정렬)
      vueProgram.ts        # .vue를 가상 .vue.ts로 투영하는 커스텀 CompilerHost
      nuxtEnvResolver.ts   # config/<mode>.json env + nuxt.config baseURL
      vueApiCallResolver.ts# $axios/$nuxt.$axios 인식 (apiCallResolver 확장)
      vueBodyWalker.ts     # 메서드/액션 본문 → http/dispatch 수집
      vuexResolver.ts      # Vuex 모듈/액션 노드
      vueRouteResolver.ts  # Nuxt pages/ 라우트
      vueIrBuilder.ts      # VueResolver 오케스트레이션 → IrFile[]
```

각 모듈의 헤더 주석에 백엔드/React 대응 파일이 명시되어 있습니다. 순수 계층(`model`/`graphBuilder`/
`join`/`norm`)과 `Resolver` 인터페이스는 React/Vue가 공유합니다.
